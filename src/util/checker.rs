use crate::{
    colors,
    database::PgPool,
    errors::actix::{JsonError, JsonErrorType},
    models::{app, app_token, token, user},
    util,
};
...
pub fn init() {
    println!("{}INIT{} checker.rs", colors::GREEN, colors::RESET);

    EMAIL_REGEX
        .set(Regex::new(r"^\w+[\+\.\w-]*@([\w-]+\.)*\w+[\w-]*\.([a-z]{2,18}|\d+)$").unwrap())
        .unwrap();
    USERNAME_REGEX
        .set(Regex::new(r"^[a-zA-Z0-9_\-]+$").unwrap())
        .unwrap();
...
pub fn min_max_size<'a>(
    ...
    if len < min || len > max {
        return Err(JsonErrorType::BAD_REQUEST
            .new_error(format!("{} must be in bounds {}-{}!", name, min, max)));
    }
    Ok(())
}
...
pub fn email<'a, 'b>(name: &'a str, email: &'b str) -> Result<(), JsonError> {
    ...
    if !regex.is_match(email) {
        return Err(JsonErrorType::BAD_REQUEST
            .new_error(format!("{} must be a valid email address!", name)));
    }
    Ok(())
}
...
pub fn username<'a, 'b>(name: &'a str, username: &'b str) -> Result<(), JsonError> {
    ...
    if !regex.is_match(username) {
        return Err(
            JsonErrorType::BAD_REQUEST.new_error(format!("{} must be a valid username!", name))
        );
    }
    Ok(())
}
...
pub fn map_qres<'a, T>(qres: QueryResult<T>, text: &'a str) -> Result<T, JsonError> {
    qres.map_err(|e| JsonErrorType::DATABASE_ERROR.new_error(format!("{}: {}", text, e)))
}

pub fn map_opt<'a, T>(opt: Option<T>, msg: &'a str) -> Result<T, JsonError> {
    match opt {
        Some(val) => Ok(val),
        None => Err(JsonErrorType::MISSING_FIELD.new_error(format!("{}", msg))),
    }
}
...
pub fn get_con(
    ...
    pool.get().map_err(|e| {
        JsonErrorType::DATABASE_ERROR.new_error(format!(
...
pub fn hex_header<'a>(
    ...
    hex(
        ...
        map_opt(
            headers.get(name),
            format!("Missing {} header", name).as_str(),
        )?
        .to_str()
        .unwrap(),
        len,
...
pub fn hex<'a, 'b>(name: &'a str, hex: &'b str, len: usize) -> Result<Vec<u8>, JsonError> {
    util::hex::parse_to_buf(hex, len).map_err(|e| {
        JsonErrorType::BAD_REQUEST.new_error(format!("Error while parsing {}: {}", name, e))
    })
}
...
pub fn authorize(
...
) -> Result<(user::User, token::Token), JsonError> {
    let tokens = map_qres(
        token::Token::find_token(token, db_connection),
    ...
    match tokens.into_iter().next() {
        Some(token) => {
            let users = map_qres(
                user::User::find(token.user_id.clone(), db_connection),
        ...
        None => Err(JsonErrorType::BAD_CREDENTIALS.new_error(format!("Invalid token!"))),
    }
}
...
pub fn authorize_app(
    token: Vec<u8>,
    db_connection: &PgConnection,
) -> Result<(app::App, app_token::AppToken), JsonError> {
    let tokens = map_qres(
        app_token::AppToken::find_token(token, db_connection),
        "Error while selecting tokens",
    )?;
    ...
    match tokens.into_iter().next() {
        Some(token) => {
            let apps = map_qres(
                app::App::find(token.app_id.clone(), db_connection),
                "Error while selecting users",
            )?;
