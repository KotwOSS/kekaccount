...
pub async fn delete(
    ...
    if token.permissions & 0b100000000 == 0 {
        ...
        let apps = map_qres(
            app::App::find_owner(id, user.id, db_connection),
        ...
        return match apps.into_iter().next() {
            Some(app) => {
                match map_qres(
                    app_token::AppToken::delete_app(token_id, app.id, db_connection),
                    "Error while deleting app token",
                )? {
                    0 => Err(JsonErrorType::NOT_FOUND
                        .new_error(format!("App token with id '{}' not found!", delete_data.id))
                        .into()),
                    _ => Ok(web::Json(json!({
                        "success": true
                    }))),
