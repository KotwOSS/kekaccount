...
pub async fn code(code_data: web::Json<CodeData>, state: web::Data<State>, request: HttpRequest) -> Result<impl Responder> {
    ...
    let codes = map_qres(access_code::AccessCode::find_app(code_id, app.id, db_connection), "Error while selecting access codes")?;

    match codes.into_iter().next() {
        Some(code) => {
            map_qres(access_code::AccessCode::delete(code.id, db_connection), "Error while deleting access code")?;

            let access_tokens = map_qres(token::Token::find(code.token_id, db_connection), "Error while selecting tokens")?;
