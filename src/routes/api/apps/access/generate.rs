...
use crate::models::{access_code, app, token, user};
use crate::util::{
...
pub async fn generate(
    ...
    generate_data: web::Json<GenerateData>,
    ...
    let users = match generate_data.username.clone() {
        ...
        None => map_qres(
            user::User::find_email(
                generate_data.email.clone().unwrap(),
                password,
                db_connection,
            ),
            "Error while selecting users",
    ...
    match users.into_iter().next() {
        Some(user) => {
            let apps = map_qres(
                app::App::find(app_id, db_connection),
            ...
            match apps.into_iter().next() {
                Some(app) => {
                    ...
                    let new_access_code = access_code::AccessCode {
                        id: access_code,
                        token_id: id.clone(),
                        app_id: app.id,
                    };
                    map_qres(
                        new_access_code.create(db_connection),
                        "Error while inserting access code",
                    )?;
                    ...
                    let new_token = token::Token {
                        ...
                        name,
                        user_id: user.id,
                        permissions: generate_data.permissions,
                    };
                    map_qres(
                        new_token.create(db_connection),
                        "Error while inserting token",
                    )?;
                ...
                None => Err(JsonErrorType::NOT_FOUND
                    .new_error(format!("App with id '{}' not found!", app_id_hex))
                    .into()),
            }
        }
        None => Err(JsonErrorType::BAD_CREDENTIALS
            .new_error(format!(
                ...
                if generate_data.username.is_some() {
                    "username"
                } else {
                    "email"
                }
