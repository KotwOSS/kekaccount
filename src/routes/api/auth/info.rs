...
#[post("/api/auth/info")]
pub async fn info(info_data: web::Json<InfoData>, state: web::Data<State>) -> Result<impl Responder> {
    ...
    let users = match info_data.username.clone() {
        ...
        None => map_qres(user::User::find_email(info_data.email.clone().unwrap(), password, db_connection), "Error while selecting users")
    }?;
    ...
    match users.into_iter().next() {
        ...
        None => Err(JsonErrorType::BAD_CREDENTIALS.new_error(format!(
                ...
                if info_data.username.is_some() {"username"} else {"email"}
            )).into())
    }
}
