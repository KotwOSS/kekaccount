...
pub async fn register(
    ...
    let password = util::hex::parse_to_buf(register_data.password.as_str(), 128).map_err(|e| {
        JsonErrorType::BAD_REQUEST.new_error(format!("Error while parsing field password! ({})", e))
    })?;
    ...
    match map_qres(
        user::User::count_name_or_email(name.clone(), email.clone(), db_connection),
        ...
        _ => Err(JsonErrorType::EXISTS
            .new_error(format!("A user with that name or email already exists!"))
            .into()),
    }
}
