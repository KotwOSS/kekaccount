...
pub async fn verify(
    verify_data: web::Json<VerifyData>,
    ...
    let users = match verify_data.username.clone() {
        ...
        None => map_qres(
            user::User::find_email(verify_data.email.clone().unwrap(), password, db_connection),
            "Error while selecting users",
        ),
    }?;
    ...
    match users.into_iter().next() {
        Some(user) => {
            let verifications = map_qres(
                verification::Verification::find_owner(id, user.id, db_connection),
                "Error while selecting verifications",
            )?;

            match verifications.into_iter().next() {
                Some(verification) => {
                    map_qres(
                        verification::Verification::delete(verification.id, db_connection),
                        "Error while deleting verification",
                    )?;
        ...
        None => Err(JsonErrorType::BAD_CREDENTIALS
            .new_error(format!(
                ...
                if verify_data.username.is_some() {
                    "username"
                } else {
                    "email"
                }
