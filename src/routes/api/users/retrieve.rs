...
#[get("/api/users/{id}")]
pub async fn retrieve(path: web::Path<(String,)>, state: web::Data<State>) -> Result<impl Responder> {
    let id_hex = path.into_inner().0;
    let id = checker::hex("id", id_hex.as_str(), 32)?;

    let db_connection = &checker::get_con(&state.pool)?;

    let users = map_qres(user::User::find(id, db_connection), "Error while selecting users")?;
