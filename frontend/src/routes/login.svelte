<script lang="ts">
	...
	async function confirm_access(e) {
		e.preventDefault();
		...
		let identifier = get_identifier(uoe.value, hashed_password);
		try {
			await Routes.Auth.INFO.send({ identifier });
...
<div class="root">
	<form on:submit={login} class="inner">
		<main class="blend-in" class:confirm={action}>
			{#if action}
				<h1>Confirm access</h1>
				{#if action.warning}<h3 class="warning">{action.warning}</h3>{/if}
				{#if action.hint}<h3 class="hint">{action.hint}</h3>{/if}
				<input
					bind:this={uoe}
					value={username ?? ""}
				...
				{/if}
				<button on:click={confirm_access} class:active={submit_loading} disabled={submit_loading}>
					{#if submit_loading}
						<Loader />
					{:else}
						Confirm
			...
			{:else}
				...
				{#if user && !force}
					<p class="hint">
						There already exists a valid token for the user {user.name}! A new token wont be
						generated! <a href="/logout">Logout</a> <a href="/dash">Dashboard</a>
					</p>
				{/if}
