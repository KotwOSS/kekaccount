	...
	onMount(async () => {
		...
		if (tokens) {
			...
			if (params.get("d") === "1") {
				...
				if (identifier)
					try {
						await Routes.Auth.Token.DELETE.send({ id: tokens, identifier });
					} catch (e) {}
				goto("/dash/tokens");
			}
		}
	...
	<div class="inner">
		...
		{:else}
			<main class="blend-in">
						...
						<div class="actions">
							<button
								...
								on:click={() => {
									...
									localStorage.setItem("tokens", show_token.id);
									goto("/login?a=1");
								}}>Delete</button
							>
					...
					<div class="tokens blend-in">
						{#each tokens as tk}
							<div
								...
								class:active={tk.active}
								class="token border hhigh"
							>
								<h2>{tk.name}</h2>
								<p>{tk.id}</p>
							</div>
				...
				<button
					on:click={refresh_tokens}
					class="refresh"
					class:active={tokens_loading}
					disabled={tokens_loading}
				>
					{#if tokens_loading}
						<Loader />
					{:else}
