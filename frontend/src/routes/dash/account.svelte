<script lang="ts">
	...
	import { APIError, get_identifier_ls, get_login_redirect, Routes } from "../../api";
	...
	onMount(async () => {
		...
		if (params.get("d") === "1") {
			let identifier = get_identifier_ls();
			if (identifier)
				try {
	...
	async function update() {
		...
		let update = Object.entries(changes)
			.filter((e) => e[1] != user[e[0]])
			.reduce((m, e) => ((m[e[0]] = e[1]), m), {});
		...
		{:else}
			<main class="blend-in">
				...
				<div class="profile border">
					...
					<div class="update">
						<button
							class:active={update_loading}
							disabled={!changed || update_loading}
							on:click={update}
						>
							{#if update_loading}
								<Loader />
				...
				<button class="delete" on:click={() => goto("/login?a=0")}>Delete</button>
			</main>
		{/if}
	</div>
</div>
